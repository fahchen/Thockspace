...
/// X: left-right (-0.3 to +0.3), Y: row height (±0.05), Z: -0.5 (constant depth)
enum KeyPositionMap {
    struct Position {
        let x: Float
        let y: Float
    ...
    private static let depth: Float = -0.5

    // Row definitions: (macKeyCode, normalizedX from 0.0 to 1.0)
    // Number row (y = +0.04)
    // QWERTY row (y = +0.02)
    // Home row (y = 0.0)
    // Bottom row (y = -0.02)
    ...
    static let positions: [CGKeyCode: Position] = {
        ...
        // Helper: distribute keys across x range
        func addRow(_ keys: [CGKeyCode], y: Float) {
            let count = keys.count
            for (i, key) in keys.enumerated() {
                let normalized = count > 1 ? Float(i) / Float(count - 1) : 0.5
                let x = -0.28 + normalized * 0.56  // -0.28 to +0.28
                map[key] = Position(x: x, y: y, z: depth)
            }
        }

        // Escape + F-keys
        addRow([53, 122, 120, 99, 118, 96, 97, 98, 100, 101, 109, 103, 111], y: 0.06)

        // Number row: ` 1 2 3 4 5 6 7 8 9 0 - = delete
        addRow([50, 18, 19, 20, 21, 23, 22, 26, 28, 25, 29, 27, 24, 51], y: 0.04)
        ...
        addRow([57, 0, 1, 2, 3, 5, 4, 38, 40, 37, 41, 39, 36], y: 0.0)

        // Bottom row: lshift Z X C V B N M , . / rshift
        addRow([56, 6, 7, 8, 9, 11, 45, 46, 43, 47, 44, 60], y: -0.02)

        // Space row: fn ctrl opt cmd space cmd opt arrows
        addRow([63, 59, 58, 55, 49, 54, 61, 123, 125, 124], y: -0.04)

        // Arrow cluster
        map[126] = Position(x: 0.28, y: -0.02, z: depth)  // Up
        map[125] = Position(x: 0.28, y: -0.04, z: depth)  // Down
        map[123] = Position(x: 0.25, y: -0.04, z: depth)  // Left
        map[124] = Position(x: 0.31, y: -0.04, z: depth)  // Right
